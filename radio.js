...
// --- ЛОГИКА ОБНОВЛЕНИЯ HUD КНОПКИ ---
window.updateRadioBadges = function() {
    // 1. Считаем непрочитанные от игроков
    let unreadPlayers = 0;
    if (window.privateChats) {
        window.privateChats.forEach(c => {
            unreadPlayers += (c.unread || 0);
        });
    }
